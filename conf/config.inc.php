...
if ($CFG->dbtype !== 'pgsql' and $CFG->dbtype !== 'postgres7') {
    die();
}

if (!empty($CFG->dboptions['dbsocket']) and ($CFG->dbhost === 'localhost' or $CFG->dbhost === '127.0.0.1')) {
    $conf['servers'][0]['host'] = '';   // custom socket not supported, sorry
    $conf['servers'][0]['port'] = 5432; // not used, might help though
} else {
    $conf['servers'][0]['host'] = $CFG->dbhost;
    if (!empty($CFG->dboptions['dbport'])) {
        $conf['servers'][0]['port'] = $CFG->dboptions['dbport'];
    } else {
        $conf['servers'][0]['port'] = 5432;
    }
}

$conf['servers'][0]['moodledbuser'] = $CFG->dbuser; // allow only our moodle username

$conf['servers'][0]['defaultdb'] = $CFG->dbname;
...
$conf['servers'][0]['pg_dumpall_path'] = '';
	...
	$conf['servers'][0]['desc'] = 'Moodle PostgreSQL DB';
